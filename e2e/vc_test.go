...
func TestVINVCLatest(t *testing.T) {
	...
	chConn, err := connect.GetClickhouseConn(&cfg)
	require.NoError(t, err)
	indexService := indexrepo.New(chConn, services.S3Server.GetClient())
	if err = StoreSampleVC(context.Background(), indexService, services.S3Server.GetVCBucket(), testVC); err != nil {
		t.Fatalf("Failed to store sample VC: %v", err)
	}
