...
func SetAggregationField(aggregations *SignalAggregations, signal *AggSignal, aliasToName map[string]string) {
	if aggregations == nil || signal == nil {
		return
	}
	switch aliasToName[signal.Alias] {
	case "angularVelocityYaw":
	...
	case "dimoAftermarketNSAT":
		aggregations.ValueNumbers[signal.Alias] = signal.ValueNumber
	case "dimoAftermarketSSID":
		aggregations.ValueStrings[signal.Alias] = signal.ValueString
	case "dimoAftermarketWPAState":
		aggregations.ValueStrings[signal.Alias] = signal.ValueString
	case "exteriorAirTemperature":
		aggregations.ValueNumbers[signal.Alias] = signal.ValueNumber
